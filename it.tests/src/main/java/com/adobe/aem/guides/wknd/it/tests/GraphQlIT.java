 ...
 */
package com.asset.distribution.it.tests;
...
public class GraphQlIT {
    ...
    private static final String assetdistribution_SHARED_GRAPHQL_ENDPOINT = "/content/_cq_graphql/assetdistribution-shared/endpoint.json";
    ...
    private static AEMHeadlessClient getHeadlessClient(InstanceConfiguration instanceConfig) throws URISyntaxException {
        final String graphQLEndpoint = instanceConfig.getUrl() + assetdistribution_SHARED_GRAPHQL_ENDPOINT;
    ...
    public void testPersistedQuery() {
        ...
        GraphQlResponse response = headlessClientAuthor.runPersistedQuery("/assetdistribution-shared/adventures-all");
        assertNull(response.getErrors());
        JsonNode responseData = response.getData();
    ...
    public void testListPersistedQueries() {
        ...
        List<PersistedQuery> listPersistedQueries = headlessClientAuthor.listPersistedQueries("assetdistribution-shared");

        assertFalse(listPersistedQueries.isEmpty());
        PersistedQuery adventuresQuery = listPersistedQueries.stream()
                .filter(p -> p.getShortPath().equals("/assetdistribution-shared/adventures-all")).findFirst().get();
        assertEquals("/assetdistribution-shared/settings/graphql/persistentQueries/adventures-all", adventuresQuery.getLongPath());
